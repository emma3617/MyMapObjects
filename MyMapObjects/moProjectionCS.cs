...
namespace MyMapObjects
{
    public class moProjectionCS
    {
        ...
        private void InitializeVars()
        {
            ...
            switch (_ProjType)
            {
                case moProjectionTypeConstant.None:
                    // No specific initialization required
                    break;
                case moProjectionTypeConstant.Mercator:
                    ak = a * _ScaleFactor;
                    break;
                case moProjectionTypeConstant.Gauss_Kruger:
                    m0 = mlfn(phi0);
                    _e = (a - b) / (a + b);
                    _e2 = Math.Pow(_e, 2);
                    _e3 = Math.Pow(_e, 3);
                    _e4 = Math.Pow(_e, 4);
                    break;
                case moProjectionTypeConstant.UTM:
                    // UTM 投影的必要變量已初始化在通用部分，不需要額外操作
                    break;
                case moProjectionTypeConstant.Lambert_Conformal_Conic_2SP:
                    double m1, m2;
                    double t, t1, t2;
                    ...
                    r0 = af0 * Math.Pow(t, n);
                    break;
                case moProjectionTypeConstant.Albers_Equal_Area:
                    double sinPhi1 = Math.Sin(ToRadians(_StandardParallelOne));
                    double sinPhi2 = Math.Sin(ToRadians(_StandardParallelTwo));
                    double cosPhi1 = Math.Cos(ToRadians(_StandardParallelOne));

                    n = 0.5 * (sinPhi1 + sinPhi2);
                    c = cosPhi1 * cosPhi1 + 2 * n * sinPhi1;
                    rho0 = a * Math.Sqrt(c - 2 * n * Math.Sin(phi0)) / n;
                    break;
                default:
                    throw new NotImplementedException($"投影類型 '{_ProjType}' 尚未實現");
            }
        }

        private moPoint AlbersFwd(moPoint LngLat)
        {
            double lng = ToRadians(LngLat.X);
            double lat = ToRadians(LngLat.Y);
            double sinLat = Math.Sin(lat);
            double theta = n * (lng - lambda0);
            double rho = a * Math.Sqrt(c - 2 * n * sinLat) / n;
            double x = _FalseEasting + rho * Math.Sin(theta);
            double y = _FalseNorthing + rho0 - rho * Math.Cos(theta);
            moPoint sResult = new moPoint(x, y);
            return sResult;
        }

        // 将投影坐标转换为经纬度——Albers Equal Area
        private moPoint AlbersInv(moPoint ProjCo)
        {
            double x = ProjCo.X - _FalseEasting;
            double y = rho0 - (ProjCo.Y - _FalseNorthing);
            double rho = Math.Sqrt(x * x + y * y);
            double theta = Math.Atan2(x, y);
            double lat = Math.Asin((c - (rho * rho * n * n) / (a * a)) / (2 * n));
            double lng = lambda0 + theta / n;
            moPoint sResult = new moPoint(ToDegrees(lng), ToDegrees(lat));
            return sResult;
        }
        // 将投影坐标转换为经纬度——Mercator
        private moPoint MercatorInv(moPoint ProjCo)
        {
            double x = ProjCo.X - _FalseEasting;
            double y = ProjCo.Y - _FalseNorthing;
            double lng = lambda0 + x / ak;
            double lat = 2.0 * Math.Atan(Math.Exp(y / ak)) - HALF_PI;
            moPoint sResult = new moPoint(ToDegrees(lng), ToDegrees(lat));
            return sResult;
        }
