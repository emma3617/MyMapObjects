...
namespace MyMapObjects
{
    public class moMapLayer
    {
        ...
        private bool _Visible = true; //图层是否显示/可见
        private bool _Selectable = true; //图层是否参与选择操作
        private string _Description = ""; //图层描述信息
        private bool _IsDirty = false; //图层是否被用户修改过，方便外部程序使用

        private moFields _AttributeFields = new moFields(); //字段集合对象
        private moFeatures _Features = new moFeatures(); //要素几何对象
        private moFeatures _SelectedFeatures = new moFeatures(); //选择要素集合
        private moRectangle _Extent = new moRectangle
            (double.MaxValue, double.MinValue, double.MaxValue, double.MinValue); //图层范围
        ...
        {
            throw new NotImplementedException(); // Won't doing the implementation in our class
        }
        ...
        public moFeatures SearchByBox(moRectangle selectingBox, double tolerance)
        {
            ...
            {
                //框选（出于简化，仅考虑相交模式，并忽略容限）
                sSelection = SearchFeaturesByBox(selectingBox);
            }
            else
            {
                throw new Exception("Invalid selection box!");
            }
            return sSelection;
        }
        ...
        {
            //说明：selectMethod对应：0-新建选择，1-添加到当前选择，2-从当前选择中选择，3-从当前选择中移除
            //出于简化，不再实现selectMethod枚举类型，且当前代码仅实现selectMethod=0的情况
            if (selectMethod == 0)
            {
                _SelectedFeatures.Clear();
        ...
        /// <param name="mpu">一个地图坐标单位对应的米数</param>
        ...
        internal void DrawSelectedFeatures(Graphics g, moRectangle extent, double mapScale, double dpm,
            double mpu, moSymbol symbol)
        {
            //判断是否在绘制范围内，如果是则绘制
        ...
        /// <param name="dpm"></param>
        /// <param name="mpu"></param>
        /// <param name="placedLabelExtents">当前已经配置的所有图层的所有注记的屏幕坐标下的范围集合，用于冲突检测</param>
        internal void DrawLabels(Graphics g, moRectangle extent, double mapScale, double dpm, double mpu,
            List<RectangleF> placedLabelExtents)
        {
            // Polygon：放在右上角
            // MultiPolyline：Polyline的每个部分都放一个注记
            // MultiPolygon：找面积最大的环放置
        ...
        /// <param name="tolerance"></param>
        /// <returns></returns>
        public moFeatures SearchByPoint(moPoint point, double tolerance)
        {
            ...
            for (Int32 i = 0; i <= sFeatureCount - 1; i++)
            {
                if (_ShapeType == moGeometryTypeConstant.Point)
                {
                    ...
                    if (moMapTools.IsPointOnPoint(point, sPoint, tolerance))
                    {
                ...
                else if (_ShapeType == moGeometryTypeConstant.MultiPolyline)
                {
                    ...
                    if (moMapTools.IsPointOnMultiPolyline(point, sMultiPolyline, tolerance))
                    {
                        sSelectedFeatures.Add(_Features.GetItem(i));
                    }
                }
                else if (_ShapeType == moGeometryTypeConstant.MultiPolygon)
                {
                    ...
                    if (moMapTools.IsPointWithinMultiPolygon(point, sMultiPolygon))
                    {
                        sSelectedFeatures.Add(_Features.GetItem(i));
                    }
                }
        ...
        {
            if (_Features == null || _Renderer == null)
                return;
            ...
            else if (_Renderer.RendererType == moRendererTypeConstant.UniqueValue)
            {
                ...
                if (sFieldIndex >= 0)
                {
                    for (Int32 i = 0; i < sFeatureCount; i++)
                    {
                        var attributeValue = _Features.GetItem(i).Attributes.GetItem(sFieldIndex);
                        if (attributeValue != null)
                        {
                            string sValueString = GetValueString(attributeValue);
                            _Features.GetItem(i).Symbol = sRenderer.FindSymbol(sValueString);
                        }
                        else
                        {
                            _Features.GetItem(i).Symbol = sRenderer.DefaultSymbol;
                        }
                    }
                }
                else
            ...
            else if (_Renderer.RendererType == moRendererTypeConstant.ClassBreaks)
            {
                ...
                if (sFieldIndex >= 0)
                {
                    ...
                    for (Int32 i = 0; i < sFeatureCount; i++)
                    {
                        ...
                        if (attributeValue != null)
                        {
                            double sValue = Convert.ToDouble(attributeValue);
