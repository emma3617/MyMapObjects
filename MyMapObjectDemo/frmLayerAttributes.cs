    ...
    public partial class frmLayerAttributes : Form
    {
        ...
        private void ShowAttributes()
        {
            ...
            for (int i = 0; i < _Layer.Features.Count; i++)
            {
                ...
                for (int j = 0; j < dgvAttributes.Columns.Count; j++)
                {
                    DataGridViewTextBoxCell cell = new DataGridViewTextBoxCell();

                    if (j < feature.Attributes.Count)
                    {
                        cell.Value = feature.Attributes.GetItem(j).ToString();
                    }
                    else
                    {
                        cell.Value = string.Empty; // 將多餘的單元格設置為空值
                    }
